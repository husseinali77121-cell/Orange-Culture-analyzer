...
# ==========================================
# 📋 قاعدة بيانات المضادات والبروتوكول الكلوي
# ==========================================
ABX_GUIDELINES = {
...
if file:
    ...
    with col1:
        ...
        # نظام تفعيل فحص الكلى
        is_renal_patient = st.checkbox("🚩 هل المريض يعاني من قصور كلوي؟")
        
        crcl = 100 # القيمة الافتراضية للشخص السليم
        
        if is_renal_patient:
            st.info("يرجى إدخال البيانات التالية لحساب التصفية:")
            weight = st.number_input("الوزن (kg)", value=75, min_value=10)
            ...
            color = "orange" if crcl > 30 else "red"
            st.markdown(f"تصفية الكرياتينين المحسوبة: <b style='color:{color}'>{crcl:.1f} ml/min</b>", unsafe_allow_html=True)
        else:
            st.success("حالة الكلى مفترضة كـ (سليم) لعدم تفعيل الخيار.")

    with col2:
        st.subheader("💊 الأدوية الحساسة (Sensitive)")
        
        # دمج الأدوية المكتشفة مع إمكانية التعديل اليدوي
        manual = st.multiselect("➕ المضادات المكتشفة (يمكنك الإضافة أو الحذف):", 
                               options=sorted(list(ABX_GUIDELINES.keys())), 
                               default=drugs)
        
        custom = st.text_input("📝 دواء غير مدرج بالقائمة؟ اكتب اسمه:")
        final_list = list(set(manual + ([custom] if custom else [])))
        
        st.divider()
        
        if not final_list:
            st.warning("يرجى اختيار الأدوية الحساسة للبدء في التحليل.")
        
        for d in final_list:
            info = ABX_GUIDELINES.get(d)
            if info:
                # التحقق من الحمل أولاً
                if is_preg and any(x in d.lower() for x in ["cipro", "levo", "oflox", "tetra", "doxy"]):
                    with st.expander(f"❌ {d} - ممنوع"):
                        st.error("🚫 خطر على الجنين (Teratogenic/Toxicity).")
                # ثم التحقق من الكلى
                elif is_renal_patient and crcl <= info['renal_limit'] and info['renal_limit'] > 0:
                ...
                st.info(f"🔹 {d}: دواء مضاف يدوياً؛ يرجى مراجعة المراجع الطبية للجرعة.")
