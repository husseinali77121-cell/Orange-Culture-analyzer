...
def _safe_image(data, **kwargs) -> None:
    ...
    `use_container_width` (newer) replaced `use_column_width` (older); passing
    the wrong one raises TypeError and would crash the page."""
    try:
...
def preprocess_image(file_bytes: bytes) -> Tuple[Any, Any]:
    ...
    # extremely slow — a Cloud timeout that looks like a hang. Upscale small
...
def detect_pus_cells(text: str) -> str:
    ...
    if re.search(r"tntc|too\s+numerous|innumerable|uncountable", scope):
        return "TNTC"
  ...
  <div class="hdr-right">
    ...
    if patho_assessment:
        ...
        if fpos:
            ...
            for f in fpos[:3]:
                H.append(f'<div style="font-size:8.5pt;color:#1e8449">{_esc(_xlate_patho(f)[:80])}</div>')
        # Against factors
        if fneg:
...
if uploaded:
    ...
    _safe_image(file_bytes, caption="Preview", use_container_width=True)
    ...
    with col1:
        ...
        weight = st.number_input("Weight (kg)", min_value=5, max_value=300,
                                 value=_wt_default)
        ...
        # ═══════════════════════════════════════════════════════════════════════
            ...
            # ── AST Quality Check moved to the results page (col2), directly
            # above the AST Quality Control box, for better visibility. ─────
    ...
    with col2:
        ...
        # ── الأدوية المضافة يدوياً (فاتها OCR كلياً) — محفوظة في session state ────
        # مثل orange: تُضاف عبر selectbox واحد للدواء + selectbox للنتيجة S/I/R
        # معاً ثم زر "أضِف" (بالأسفل، بعد القائمة مباشرةً).
        _manual_key = f"manual_added_{file_hash[:8]}"
        if _manual_key not in st.session_state:
            st.session_state[_manual_key] = []
        manual_extra = [d for d in st.session_state[_manual_key] if d not in ocr_drugs]
        ...
        # ── Add-a-drug control (orange style): pick the drug AND its S/I/R
        # result together, then press add. Persisted per-file. ─────────────
        _add_already = set(ocr_drugs) | set(manual_extra) | set(ocr_detected_no_sir)
        _add_options = [d for d in all_known if d not in _add_already]
        _add_c1, _add_c2 = st.columns([5, 2])
        _pick = _add_c1.selectbox(
            "➕ أضف مضاداً فاته الـ OCR",
            options=["— اختر مضاداً —"] + _add_options,
            index=0,
            key=f"add_pick_{file_hash[:8]}",
            help="اختر اسم المضاد ونتيجته ثم اضغط أضِف — سيظهر في القائمة أعلاه",
        )
        _add_sir = _add_c2.selectbox(
            "النتيجة",
            options=sir_options,
            index=0,
            key=f"add_sir_{file_hash[:8]}",
        )
        if _add_c1.button("➕ أضِف إلى القائمة", key=f"add_btn_{file_hash[:8]}",
        ...
        # ── AST Quality Check (QC Engine) — moved ABOVE the AST Quality
        # Control box so it's clearly visible on the results page. ─────────
