...
def analyze_antibiotics(
    ...
    # Nitrofurantoin & Fosfomycin (PO) concentrate in urine only — no systemic/
    # tissue levels. They must NOT be offered for blood, CSF, sputum, wound, pus,
    # or GI infection even if the AST reports S (EUCAST/IDSA: lower-UTI only).
    _cl = culture_type.lower()
    is_stool_gi = any(k in _cl for k in ["stool", "fecal", "rectal"])
    is_urine    = "urine" in _cl
    _URINE_ONLY = {"Nitrofurantoin", "Fosfomycin"}

    if not is_urine:
        # move any urine-only agent from allowed/warned into banned with a reason
        def _demote_urine_only(bucket_name):
            keep = []
            for d in (allowed if bucket_name == "a" else warned):
                nm = d.get("name")
                if nm in _URINE_ONLY:
                    banned.append(build_banned_item(
                        nm, "specimen",
                        "عامل بولي فقط -- غير مناسب لهذه العينة.",
                        "عامل بولي فقط -- لا يحقق تركيزاً علاجياً في الدم أو الأنسجة؛ غير "
                        "مناسب للعدوى الجهازية (دم / رئة / جرح / صديد / سائل نخاعي) ولا "
                        "لالتهاب الكلية الصاعد، حتى لو أظهرت المزرعة حساسية. "
                        "(ملاحظة: Fosfomycin الوريدي استثناء غير متوفر في هذه القائمة.)",
                    ))
                else:
                    keep.append(d)
            return keep
        allowed = _demote_urine_only("a")
        warned  = _demote_urine_only("w")
        # Norfloxacin: a urinary FQ — for GI (stool) it has an enteric role, but
        # for systemic specimens it is not appropriate. Keep existing behaviour
        # (handled by AST/other rules); only Nitro/Fosfo are specimen-banned here.
