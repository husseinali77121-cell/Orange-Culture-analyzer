...
def generate_decision_tree_image(
    ...
    p_lines = []
    if patient_name:
        # Pillow لا يدعم Arabic — نكتب "Patient" كـ label بدون الاسم العربي
        try:
            patient_name.encode("ascii")
            p_lines.append(f"Name: {patient_name}")
        except UnicodeEncodeError:
            # اسم عربي: نحاول نكتب الحروف اللاتينية إن وجدت، وإلا نكتب "Patient"
            safe = patient_name.encode("ascii", errors="ignore").decode().strip()
            p_lines.append(f"Patient: {safe}" if safe else "Patient (Arabic Name)")
