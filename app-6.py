...
def generate_decision_tree_image(
        ...
        for item in items:
            ...
            for b in ["[A]", "[W]"]:
                if item.endswith(b):
                    ...
                    break
            # ارسم الاسم كاملاً أولاً
            cy = text_wrap(draw, x1 + 14*S, cy, f"• {display_name}",
                           fi, DARK, x2 - x1 - 26*S, gap=3)
            # ارسم الـ badge على سطر منفصل تحت الاسم
            if badge:
                badge_clr = BADGE_COLORS.get(badge, DARK)
                badge_lbl = "ACCESS" if badge == "[A]" else "WATCH"
                blw = int(tw(draw, badge_lbl, fs))
                badge_h = fh(fs) + 4*S
                bx = x1 + 26*S
                if cy + badge_h + 4*S < y2 - 8*S:
                    draw.rounded_rectangle(
                        (bx, cy, bx + blw + 8*S, cy + badge_h),
                        radius=3*S, fill=badge_clr
                    )
                    draw.text((bx + 4*S, cy + 1*S), badge_lbl, fill=WHITE, font=fs)
                    cy += badge_h + 4*S
            cy += 3*S
    ...
    htxt = f"Orange Lab — CLINICAL DECISION TREE  |  {lab_name.upper()}"
    if lab_city:
        htxt += f"  |  {lab_city}"
    ...
    p_lines = []
    if patient_name:
        # Pillow لا يدعم Arabic RTL — نعرض الاسم اللاتيني أو نشير لوجود اسم عربي
        try:
            patient_name.encode("ascii")
            p_lines.append(f"Name: {patient_name}")
        except UnicodeEncodeError:
            safe = patient_name.encode("ascii", errors="ignore").decode().strip()
            p_lines.append(f"Name: {safe if safe else '[Arabic Name]'}")
    ...
    draw.text((AB[0]+12*S, 72*S+12*S), "IMPORTANT ALERT", fill=ALERT_TXT, font=F_SUBTITL)
    alerts: List[str] = []
    ...
    if "klebsiella" in org_l:
        ...
        alerts += ["High intrinsic resistance", "Use anti-pseudomonal agents only"]
    elif "mrsa" in org_l or "staphylococcus" in org_l:
        alerts += ["Gram-positive organism", "Check methicillin resistance (MRSA)"]
    elif "acinetobacter" in org_l:
        alerts += ["High MDR risk", "Check carbapenem sensitivity"]
    if is_renal:
        alerts.append(f"Renal adj. needed (CrCl {cl_cr:.0f} ml/min)")
    if is_preg and age >= 18:
        alerts.append("Pregnancy: verify fetal safety")
    if age < 18:
        alerts.append("Pediatric: verify age suitability")
    if not alerts:
    ...
    for al in alerts[:5]:
        if ay + fh(F_SMALL) + 4*S > AB[3] - 6*S:
            break
        ay = text_wrap(draw, AB[0]+12*S, ay, f"• {al}", F_SMALL, DARK, AB[2]-AB[0]-22*S, gap=4)
        ay += 2*S
...
def generate_pdf_report(
    ...
    header_data = [[
        ...
        Paragraph(f"<font size='7'>{now_str}</font>",
                  S("d", fontName=F, fontSize=7, textColor=colors.HexColor("#B0C4DE"),
                    alignment=TA_RIGHT)),
    ]]
    ...
    culture_cell = [
        Paragraph(f'<b><font color="#04193F">{specimen.upper()} CULTURE RESULT</font></b>',
                  S("ch", fontName=FB, fontSize=10, leading=14, textColor=NAVY, alignment=TA_CENTER)),
        Paragraph(f'<font size="18" color="#04193F">{organism}</font>',
                  S("org", fontName=FB, fontSize=18, leading=22, textColor=NAVY, alignment=TA_CENTER)),
        Spacer(1, 2*mm),
    ]
    for ln in [cc_line, di_line, pu_line, rb_line]:
        if ln:
            culture_cell.append(Paragraph(ln, S("cc", fontName=F, fontSize=8.5, leading=12,
                                                  textColor=GRAY, alignment=TA_CENTER)))

    top_table = Table([[patient_cell, culture_cell]], colWidths=[68*mm, 112*mm])
    ...
    for title_txt, detail_txt, clr, clrL in alert_rows:
        at = Table([[
            Paragraph(f"<b>{title_txt}</b>", S("al", fontName=FB, fontSize=9, leading=13, textColor=clr)),
            Paragraph(detail_txt,             S("ad", fontName=F,  fontSize=8, leading=12, textColor=clr)),
    ...
    if sir_map:
        ...
        if len(drugs_list) > 6:
            ...
            def mini_sir_table(items):
                rows = [[Paragraph("Antibiotic", sBold), Paragraph("S/I/R", sBold)]]
                for drug, res in items:
                    clr = SIR_COLORS.get(res, GRAY)
                    rows.append([
                        Paragraph(drug, sNormal),
                        Paragraph(f'<b>{res} — {SIR_LABELS_PDF.get(res,res)}</b>',
            ...
            sir_data = [[Paragraph("<b>Antibiotic</b>", sBold),
            ...
            for drug, result in sorted(sir_map.items()):
                ...
                sir_data.append([
                    Paragraph(drug, sNormal),
                    Paragraph(f'<b>{result}</b>', S("sr", fontName=FB, fontSize=9, leading=13, textColor=clr)),
                    Paragraph(SIR_LABELS_PDF.get(result, result),
                              S("sl", fontName=F, fontSize=8.5, leading=12, textColor=clr)),
                ])
    ...
    if allowed:
        ...
        for item in allowed:
            ...
            route_short = "PO" if item.get("high_po") else "IV/IM"
            sir_txt = f" [<b>{sir}</b>]" if sir else ""
            aware_color = {"Access":"green","Watch":"orange","Reserve":"red"}.get(item.get("aware",""),"black")
            note_short = (note or "")[:60] + ("..." if len(note or "") > 60 else "")
            row = [
                Paragraph(f"<b>{item['name']}</b>{sir_txt}", sNormal),
                Paragraph(f'<font color="{aware_color}"><b>{item.get("aware","")}</b></font>', sNormal),
                Paragraph(route_short, sNormal),
                Paragraph(note_short, sSmall),
            ]
            if is_renal:
                row.append(Paragraph((item.get("renal_note",""))[:50], sSmall))
            rec_data.append(row)
    ...
    if warned:
        ...
        for item in warned:
            ...
            if item.get("warning_reason") == "intermediate_culture":
                ...
                action = "Use only if no better alternative"
            else:
    ...
    if banned:
        ...
        story.append(Spacer(1, 2*mm))
        cat_label = {
            "resistant": "Resistant (Culture)", "renal": "Renal Contraindication",
            "pregnancy": "Pregnancy - Banned",  "child": "Not Suitable for Age",
            "organism":  f"Ineffective for {organism}", "other": "Other",
        }
    ...
    if interactions:
        ...
        for alert in sorted(set(interactions)):
            story.append(Paragraph(f"• {cpdf(alert)}", sItem))
        story.append(Spacer(1, 3*mm))
    ...
    footer_data = [[
        Paragraph("DISCLAIMER: This report is a clinical decision support tool. "
                  "Final therapeutic decisions remain with the treating physician.",
                  S("disc", fontName=FI, fontSize=7.5, leading=11, textColor=GRAY)),
        Paragraph(f"<b>Guidelines:</b> EUCAST 2026 | CLSI M100 2026 | IDSA AMR 2025<br/>"
                  f"WHO AWaRe 2025 | Egypt National | BNF 2025",
                  S("ref", fontName=F, fontSize=7, leading=10, textColor=GRAY, alignment=TA_RIGHT)),
    ]]
    ...
    def add_page_number(canvas_obj, doc_obj):
        ...
        canvas_obj.drawRightString(A4[0] - 15*mm, 7*mm, f"{lab_name}  |  Page {pg}")
...
if uploaded:
        ...
        if mdr_result["level"] or (esbl_result.get("probability") and
            ...
            with st.expander("🧬 Resistance Classification", expanded=True):
                ...
                if prob == "carbapenemase":
                    ...
                    st.error(
                        "**High Probability ESBL Producer**\n" +
                        esbl_result["detail"] + "  \n🔹 " + esbl_result["action"]
                    )
                elif prob == "moderate":
                    st.warning(
        ...
        if final_drugs:
            ...
            if PIL_AVAILABLE:
                try:
                    ...
                    st.image(img_bytes,
                             caption=f"Orange Lab — Clinical Decision Tree  |  {_pt}  |  {str(date_in)}",
                             use_container_width=True)
  ...
  target="_blank"
  style="
    display:block; text-align:center; padding:0.45rem 1rem;
    background:#1B4F9E; color:white; border-radius:8px;
    font-size:0.95rem; font-weight:600; text-decoration:none; line-height:2;
